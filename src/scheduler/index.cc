...
int pick_tile_factor(const int extent, const int l1_bytes = 32768) {
	const int max_floats = l1_bytes / static_cast<int>(sizeof(float)); // 8192
	
	const int max_tile = static_cast<int>(std::sqrt(max_floats));
	for (int f = max_tile; f >= 4; f--)
		if (extent % f == 0)
			return f;
	return 1;
}
	...
	if (!(flags & NO_TILE)) {
		for (auto &nest : nests) {
			...
			for (auto &name : nest.order) {
				if (name.ends_with("_outer"))
					outers.push_back(name);
				else if (name.ends_with("_inner"))
					inners.push_back(name);
				else
					reductions.push_back(name);
			}
			std::vector<std::string> new_order;
			new_order.insert(new_order.end(), outers.begin(), outers.end());
			new_order.insert(new_order.end(), reductions.begin(),
							 reductions.end());
