...
ExprPtr make_index_expr(const std::string &idx_name, const LoopNest &nest) {
	if (nest.tiled.count(idx_name)) {
		auto &[outer, inner] = nest.tiled.at(idx_name);

		int factor = -1; // sentinel — -1 means not found
...
ExprPtr make_array_access(const TensorAccess &access, const LoopNest &nest) {
	...
	for (int i = 0; i < (int)access.indices.size(); i++) {
		int stride = 1;
...
std::vector<StmtPtr> lower_compute(const Compute &compute,
	...
	switch (compute.op) {
	case Op::MATMUL: {
		auto mm = lower_matmul(compute, nest);
		stmts.push_back(mm);

		break;
	}
	case Op::BIAS_ADD: {
		auto ba = lower_ba(compute, nest);
		stmts.push_back(ba);

		break;
	}
	case Op::RELU: {
		auto rl = lower_relu(compute, nest);
		stmts.push_back(rl);
		break;
	}
	default:
